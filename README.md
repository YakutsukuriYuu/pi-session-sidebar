...
| 元素 | 图标 | 颜色 | 光标颜色（被选中时） |
| --- | --- | --- | --- |
| **文件夹** | `▣`（展开）/ `▢`（折叠），在第 2 列 | **蓝色加粗** | 蓝色 |
| **当前项目** | 同上，左侧多一个蓝色 `▌` 竖条 | 蓝色加粗 | 蓝色 |
| **当前会话** | `●` | **青色** | 青色 |
| **其他会话** | `│`（引导线，宽时） | 默认色 | 亮白 |

两条规则让两者永远不会混淆：

1. **只有被选中的行才有 `->>` 光标**——所以“看得到箭头 = 按 Enter 能切过去”是一条可靠规则
...
▌▣ pi-plugin             3
  ● 修复登录 bug  14:23     ← 普通行
->> 插件设计讨论   9/3       ← 选中行：箭头 + 行类型颜色
    快捷键自定义
```

- 光标**取代**原有的标记位（不是额外插在前面），所以选中时**标题列不会左右移动**——
  上下移动选择时文字不抖
- 光标颜色沿用行类型：`文件夹=蓝`、`当前会话=青`、`其他会话=亮白`
- 不铺任何底色/边框属性，因此不会和行内的青色/蓝色文字冲突，也不依赖终端对
  上划线等扩展属性的支持
...
- **切换有反馈**：按 Enter 时底部立即显示 `⟳ 正在切换 → 目标会话`，切换完成后当前会话的 `●` 会**脉动两圈**再定格
- **运行中保护**：当前会话还在跑时，侧栏**拒绝切换**并提示 `当前会话正在运行，不能切换`（可用 `guardBusySession` 关闭）
...
| `guardBusySession` | **运行中保护**（默认 `true`）：当前会话还在工作时，拒绝**本插件的**切换/新建动作，避免误杀正在生成的回答。**不影响 pi 自己的 `/resume`、`/new`、`/fork`、`/tree`** |
