...
1. **只有被选中的行才有矩形框**——所以“看得到框 = 按 Enter 能切过去”是一条可靠规则
2. **会话统一缩进**在文件夹之下（标记在第 3 列，标题从第 5 列开始）

### 选中态：矩形框

选中行用**矩形框**包围，纯描边、不加底色：
...
实现方式：真正画 `┌─┐` 需要三行（上边、内容、下边），而列表里每行都被相邻会话占满。
所以上下边用**上划线（SGR 53）+ 下划线（SGR 4）属性**在**同一行内**画出，左右用半宽
竖线 `▏` / `▕`（实测单列宽）。效果等同于一个真正的矩形，且**不占用额外行、不覆盖相邻会话**。

- 框**纯描边**，没有背景色块，因此不会和行内的青色/蓝色文字冲突
- 框的列位设计保证了**标题列不会因为被选中而移动**（左边竖线占用原本的缩进列）
- 行内内容全部使用框线颜色：`文件夹=蓝`、`当前会话=青`、`其他会话=亮白`
- **上划线兼容性**：Ghostty / kitty / WezTerm / iTerm2 / foot 支持；macOS 自带
  Terminal 不支持 SGR 53，那里会退化成“只有下边 + 两条竖线”的半框
